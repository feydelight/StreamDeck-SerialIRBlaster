...
function websocketOnMessage(evt) {
    ...
    if (jsonObj.event === 'didReceiveSettings') {
        ...
        globalSettings = jsonObj.payload;
        Log('globalSettings: ' + JSON.stringify(globalSettings, null, true));

        var serialPorts = globalSettings.globalSettings.serialPortSettings;
        Log('SerialPorts: ' + JSON.stringify(serialPorts, null, true));
        if (!serialPorts) {
...
function AddSerialPort(serialPortSetting) {
    sendPayloadToPlugin({
        property_inspector: 'socket-add',
        displayName: serialPortSetting.displayName,
        comPort: serialPortSetting.comPort,
        baudRate: serialPortSetting.baudRate,
        dataBits: serialPortSetting.dataBits,
        parity: serialPortSetting.parity,
        stopBit: serialPortSetting.stopBit,
    });
}

function RemoveSerialPort(key) {
    sendPayloadToPlugin({
        property_inspector: 'socket-remove',
        key: key,
    });
}
...
function OpenSettings() {
    if (settingWindow != null && settingWindow.closed == false) {
        settingWindow.focus();
        return;
    }
    settingWindow = window.open("Setup/Setup.html");
}
